#==== Simulation II: Scenario I & n= 1000000 ========
...
for (i in 1:100){
  ...
  for (j in 1:6){
    try({
      ...
      c0 = FLoS_result$c0
      # c_Lev = Lscore(N, yc, r, lambda.FLoS, V)
...
# PCC
mPCC_1 = apply(PCC_1,c(2,3),mean)
datapcc = data.frame(r = rep(r,2), Method = Method, value = as.vector(t(mPCC_1)))
ggplot(data=datapcc, aes(x=r, y=value, group=Method,colour=Method)) +
  geom_point(aes(shape = Method))+
  geom_line(aes(linetype = Method))+
  scale_color_manual(values = c("black", "red"))+
  # ggtitle("Scenario I")+
  labs(x="subsample size", y="PCC")+
